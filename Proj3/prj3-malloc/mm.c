/*
 * Segregate List 방법 사용.
 * block의 사이즈를 오름차순으로 정렬하여 best-fit 사용.
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
/* Given block ptr bp, compute adderss of next and previous blocks */
...
int mm_init(void)
{
	...
	if((seg_list = mem_sbrk((LISTS+4)*WSIZE)) == (void *)-1)
		return -1;
	...
	heap_listp = (char*)seg_list + LISTS * WSIZE;
	...
	/* Extend the empty heap with a free block of CUHNKSIZE bytes */
...
static void *coalesce(void *bp)
{
	...
	if(prev_alloc && next_alloc) {					/* prev allocated, next allocated */
...
static void *find_fit(size_t asize)
{
	...
	while(list < LISTS) {
		if((list == LISTS - 1) || ((searchsize <= 1) && (seg_list[list] != NULL))) {
			bp = seg_list[list];
